...
def flight_plan(fp_type):
	...
	if fp_type=='xml':
		fw = open('./oneworld_flights.xml','wb')
		files=glob.glob('./airlines/*.xml')
		for f in files:
			os.unlink(f)
		
	if fp_type=='conf':
		fw = open('./oneworld_flights.conf','wb')
...
def filter_dupes():
	...
	fr=open('./oneworld_flights.conf','rb')
	content= fr.readlines()
	callsign_list=[]
	buf=''
	use=1
	...
	fw=open('./filtered_oneworld_flights.conf','wb')
	fw.write(buf)
	fw.close()		
	fr.close()				
...
if __name__ == "__main__":
	if len(sys.argv) <2:
		print 'Usage: generator.py gen | fp [ conf | xml ] | filter'
		sys.exit()
	else:
		if sys.argv[1]=='gen':
			generate()
