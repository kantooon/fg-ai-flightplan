...
'''AirFrance flight plan generator'''
...
def generate(arg):
	...
	if arg=='crop':
		for i in range(4,98):
			i=str(i)
			#col1
			os.system('pdftotext -f '+i+' -l '+i+' -r 72 -x 0 -y 0 -W 297 -H 840 -layout  AirFrance2011.pdf af/col1-'+i+'.txt')
			#col2
			os.system('pdftotext -f '+i+' -l '+i+' -r 72 -x 300 -y 0 -W 294 -H 840 -layout  AirFrance2011.pdf af/col2-'+i+'.txt')
			
		buf=''
		for i in range(4,98):
			i=str(i)
			fr1= open('./af/col1-'+i+'.txt','r')
			content1= fr1.read()
			fr2= open('./af/col2-'+i+'.txt','r')
			content2= fr2.read()
			fr1.close()
			fr2.close()
			buf=buf+content1+content2
		fw = open('./orar-airfrance.txt','wb')
		fw.write(buf)
		fw.close()
		return
	...
	for line in content:
		...
		if re.search('^\s+➔\s+.*?\s[A-Z\s]{3,5}\s+\+[0-9]{1,2}:[0-9]{1,2}',line)!=None:
			match=re.search('^\s+➔\s+.*?\s([A-Z\s]{3,5})\s+\+[0-9]{1,2}:[0-9]{1,2}',line)
			if match!=None:
				apt_iata=match.group(1)
		...
		if re.search('\s[0-9]{2}\.[0-9]{2}\s*[A-Za-z0-9]{0,1}\s+[0-9]{2}\.[0-9]{2}\s',line)!=None:
			match=re.search('\s([0-9]{2}\.[0-9]{2})\s*[A-Za-z0-9]{0,1}\s+([0-9]{2}\.[0-9]{2})\s',line)
			if match!=None:
			...
			match1=re.search('^\s*([0-9\s\-]+)\s+[0-9]{2}\.[0-9]{2}\s+',line)
			if match1!=None:
			...
			match2=re.search('\s+[0-9]{2}:[0-9]{2}\s+(AF[0-9]{2,5}\(?[A-Z0-9]{0,2}\)?)\s+→\s+',line)
			if match2!=None:
...
def flight_plan(fp_type):
	...
	if fp_type=='xml':
		fw = open('./airfrance_flights.xml','wb')
