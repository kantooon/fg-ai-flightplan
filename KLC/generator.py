...
'''KLM City hopper flight plan generator'''
...
def generate(arg):
	...
	for line in content:
		...
		if re.search('[0-9\s\-]+\s([0-9]{4})\s+([0-9]{4})\+?1?\s+(KL[0-9]{2,5})\s+KLc\s+[0-9]{1,2}:[0-9]{2}\s+0',line)!=None:
			match=re.search('([0-9\s\-]+)\s([0-9]{4})\s+([0-9]{4})\+?1?\s+(KL[0-9]{2,5})\s+KLc\s+[0-9]{1,2}:[0-9]{2}\s+0',line)
			if match!=None:
				days1=match.group(1)
				days1=days1.replace(' ','')
				days1=days1.replace('-','.')
...
def aircraft_list(req):
	aircraft_table=['100', 'F70', 'E90', '737', '733', '738']
