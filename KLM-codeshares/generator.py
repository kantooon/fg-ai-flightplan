...
'''KLM codeshares flight plan generator'''
...
def generate(arg):
	...
	for line in content:
		...
		if re.search('[0-9\s\-]+\s([0-9]{4})\s+([0-9]{4})\+?1?\s+([A-Z0-9]{1}[A-Z]{1}[0-9]{2,5})\s+([A-Z0-9]{2})\s+[0-9]{1,2}:[0-9]{2}\s+0',line)!=None:
			match=re.search('([0-9\s\-]+)\s([0-9]{4})\s+([0-9]{4})\+?1?\s+([A-Z0-9]{1}[A-Z]{1}[0-9]{2,5})\s+([A-Z0-9]{2})\s+[0-9]{1,2}:[0-9]{2}\s+0',line)
			if match!=None:
				days1=match.group(1)
				days1=days1.replace(' ','')
				days1=days1.replace('-','.')
			...
			if callsign[0:2]=='EV':
				req_aircraft='CRJ'
			elif callsign[0:2]=='BE':
				req_aircraft='DH4'
			elif callsign[0:2]=='YS':
				req_aircraft='ERJ'
			elif callsign[0:2]=='9E':
				req_aircraft='CR9'
			elif callsign[0:2]=='OH':
				req_aircraft='CRJ'
			elif callsign[0:2]=='MN':
				req_aircraft='737'
			elif callsign[0:2]=='WX':
				req_aircraft='AR8'
			elif callsign[0:2]=='VG':
				req_aircraft='F50'
			elif callsign[0:2]=='XJ':
				req_aircraft='CRJ'
			elif callsign[0:2]=='DB':
				req_aircraft='CRJ'
			elif callsign[0:2]=='XK':
				req_aircraft='AT4'
			elif callsign[0:2]=='G3':
				req_aircraft='737'
			elif callsign[0:2]=='CT':
				callsign = 'AP'+callsign[2:]
				req_aircraft='CRJ'
			else:
				req_aircraft='#'
			buf=buf+ departure +','+destination+','+ dep_time1 + ',' + arr_time1 +',' + req_aircraft +','+callsign + ',' + days1 +'\n'	
		else:
			continue
	...
	missing=[]
	completed=''
	fc=open('./timetable.txt')
	fw2=open('./timetable2.txt','wb')
	lines=fc.readlines()
	for line in lines:
		if line.find('#')!=-1:
			count+=1
...
def flight_plan(fp_type):
	...
	if fp_type=='xml':
		fw = open('./codeshares_flights.xml','wb')
	...
	if fp_type=='conf':
		fw = open('./codeshares_flights.conf','wb')
		files=glob.glob('./airlines/*.xml')
		for f in files:
			os.unlink(f)
...
def filter_dupes():
	...
	fr=open('./codeshares_flights.conf','rb')
	content= fr.readlines()
	callsign_list=[]
	buf=''
	use=1
